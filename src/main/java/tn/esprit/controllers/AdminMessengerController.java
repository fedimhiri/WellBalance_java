...
public class AdminMessengerController implements Initializable {
    ...
    @FXML
    private ScrollPane conversationsScroll;
    ...
    @FXML
    private Label statUrgent;

    @FXML
    private VBox urgentList;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        // Check if logged in
        if (!sessionManager.isLoggedIn() || !sessionManager.isDoctor()) {
            navigateToLogin();
            return;
        }
        ...
        messageInput.textProperty().addListener((obs, oldV, newV) -> {
            if (currentConversation != null) {
                ...
                if (currentConversation.isTypingDoctor() != isTyping) {
                    currentConversation.setTypingDoctor(isTyping);
    ...
    private void loadDashboardStats() {
        if (statTotalConv == null) return;
        List<Conversation> convs = conversationService.getConversationsForDoctor(sessionManager.getCurrentUserId());
        
        // Populate Top Stat Bar
        statTotalConv.setText(String.valueOf(convs.size()));
        int unread = convs.stream().mapToInt(c -> c.getUnreadCountFor(sessionManager.getCurrentUser())).sum();
        statUnread.setText(String.valueOf(unread));
        long urgentsCount = convs.stream().filter(c -> "urgence".equalsIgnoreCase(c.getType())).count();
        statUrgent.setText(String.valueOf(urgentsCount));
        
        // Detailed Analytics: Sent vs Received vs Media
        int sent = 0;
        int received = 0;
        int mediaCount = 0;
        for (Conversation c : convs) {
            List<Message> msgs = messageService.getByConversationId(c.getId());
            for (Message m : msgs) {
                if (m.getSenderId() == sessionManager.getCurrentUserId()) sent++;
                else received++;
                if (m.getAttachment() != null || "audio".equals(m.getMessageType())) mediaCount++;
            }
        }
        
        messagesPieChart.getData().clear();
        PieChart.Data d1 = new PieChart.Data("Envoyés", sent);
        PieChart.Data d2 = new PieChart.Data("Reçus", received);
        PieChart.Data d3 = new PieChart.Data("Médias", mediaCount);
        messagesPieChart.getData().addAll(d1, d2, d3);
        
        // Visual polish for PieChart: add labels and effects
        messagesPieChart.setTitle("Volume d'échanges");
        
        if (urgentList != null) {
            urgentList.getChildren().clear();
            convs.stream()
                .filter(c -> "urgence".equalsIgnoreCase(c.getType()))
                .sorted((c1, c2) -> c2.getUpdatedAt().compareTo(c1.getUpdatedAt()))
                .limit(5)
                .forEach(c -> {
                    urgentList.getChildren().add(createUrgentHubCard(c));
                });
            if (urgentList.getChildren().isEmpty()) {
                VBox emptyBox = new VBox(10);
                emptyBox.setAlignment(Pos.CENTER);
                emptyBox.setPadding(new Insets(40));
                Label icon = new Label("✨");
                icon.setStyle("-fx-font-size: 40;");
                Label empty = new Label("Aucune urgence active.");
                empty.setStyle("-fx-text-fill: #adb5bd; -fx-font-size: 14; -fx-font-weight: bold;");
                emptyBox.getChildren().addAll(icon, empty);
                urgentList.getChildren().add(emptyBox);
            }
        }
    }

    private VBox createUrgentHubCard(Conversation conv) {
        VBox card = new VBox(10);
        card.getStyleClass().add("activity-row");
        card.setStyle("-fx-background-color: white; -fx-background-radius: 15; -fx-padding: 18; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.04), 10, 0, 0, 4); -fx-border-color: #f1f3f5; -fx-border-width: 1; -fx-border-radius: 15;");
        
        HBox header = new HBox(12);
        header.setAlignment(Pos.CENTER_LEFT);
        
        Circle pulse = new Circle(5, Color.web("#ff4444"));
        
        String patientName = conv.getUser() != null ? conv.getUser().getUsername() : "Patient Anonyme";
        Label nameLabel = new Label(patientName);
        nameLabel.setStyle("-fx-font-size: 15; -fx-font-weight: bold; -fx-text-fill: #212529;");
        
        Region spacer = new Region();
        HBox.setHgrow(spacer, Priority.ALWAYS);
        
        Label timeLabel = new Label("Il y a " + formatRelativeTime(conv.getUpdatedAt()));
        timeLabel.setStyle("-fx-font-size: 11; -fx-text-fill: #adb5bd;");
        
        header.getChildren().addAll(pulse, nameLabel, spacer, timeLabel);
        
        Message lastMsg = conversationService.getById(conv.getId()).getLastMessage();
        String preview = lastMsg != null ? lastMsg.getContent() : "Aucun message récent";
        if (preview.length() > 60) preview = preview.substring(0, 57) + "...";
        
        Label previewLabel = new Label(preview);
        previewLabel.setStyle("-fx-font-size: 13; -fx-text-fill: #6c757d; -fx-font-style: italic;");
        previewLabel.setWrapText(true);
        
        HBox footer = new HBox(10);
        footer.setAlignment(Pos.CENTER_RIGHT);
        Button btnOpen = new Button("Traiter le cas");
        btnOpen.setStyle("-fx-background-color: #ff444415; -fx-text-fill: #ff4444; -fx-font-size: 11; -fx-font-weight: bold; -fx-background-radius: 8; -fx-padding: 5 12;");
        footer.getChildren().add(btnOpen);
        
        card.getChildren().addAll(header, previewLabel, footer);
    ...
    // Helper to format time relatively (simple mock for now)
    ...
    private void loadConversations() {
        ...
        List<Conversation> conversations = conversationService.getConversationsForDoctor(
    ...
    private void searchConversations(String keyword) {
        ...
        List<Conversation> conversations = conversationService.search(
            keyword, sessionManager.getCurrentUserId()
        );
    ...
    private HBox createConversationBox(Conversation conv) {
        ...
        Circle circle = new Circle(22, Color.web("#e4e6eb"));
        Label letter = new Label(conv.getUser() != null ? conv.getUser().getAvatarLetter() : "?");
        letter.getStyleClass().add("avatar-text");
        ...
        String patientName = conv.getUser() != null ? conv.getUser().getUsername() : "Patient";
        Label nameLabel = new Label(patientName);
        ...
        // Unread count
        int unread = conv.getUnreadCountFor(sessionManager.getCurrentUser());
        if (unread > 0) {
            nameLabel.setStyle("-fx-font-size: 14; -fx-font-weight: bold;");
        ...
        box.getChildren().addAll(avatar, info, spacer, dateLabel);

        // Click handler
        box.setOnMouseClicked(e -> selectConversation(conv));

        // Hover effect using standard CSS
        // No custom handlers needed, it is managed by pseudo-classes in CSS
    ...
    private void selectConversation(Conversation conv) {
        ...
        // Update chat header
        User patient = currentConversation.getUser();
        if (patient != null) {
            chatUserNameLabel.setText(patient.getUsername());
            chatAvatarLabel.setText(patient.getAvatarLetter());
        }
        chatStatusLabel.setText("Patient - " + (currentConversation.getType() != null ? currentConversation.getType() : "Normal"));

        // Load messages
        loadMessages();

        // Mark as read
        messageService.markAsRead(currentConversation.getId(), sessionManager.getCurrentUserId());

        // Refresh conversations list to update selection highlight
        loadConversations();
        loadDashboardStats();
    }
    ...
    private javafx.scene.text.TextFlow buildTextFlowWithEmojis(String text, boolean isMine) {
        ...
        // Use very explicit fill colors to avoid "white on white" issues
    ...
    private void handleAttachment(javafx.event.ActionEvent event) {
        ...
        if (selectedFile != null) {
            try {
                ...
                msg.setSenderId(sessionManager.getCurrentUserId());
    ...
    private void loadMessages() {
        ...
        // Optimization: Check if we actually need a full redraw
        // We check message count AND a checksum of content/reactions to detect changes
        if (messages.size() == messagesContainer.getChildren().size()) {
            boolean changed = false;
            // Simplified check: reactions or content might have changed even if size is same
            // For now, let's just compare the last message's reaction count as a hint
            if (!messages.isEmpty() && !messagesContainer.getChildren().isEmpty()) {
                // If we want real-time reactions to show up, we MUST redraw or update specific nodes.
                // For safety and simplicity in this workshop, we'll force redraw if any message 
                // in the list has different reactions than what's expected.
                // But to be "real-time" and responsive, let's just force redraw if loadMessages is called manually
                // (which happens after a click).
            }
        }
    ...
    private HBox createMessageBox(Message msg) {
        boolean isMine = msg.getSenderId() == sessionManager.getCurrentUserId();
        ...
        // Content (With API Emoji Support)
        ...
        bubble.setOnMouseClicked(e -> {
            if (e.getClickCount() == 2) {
                ...
                String reactorName = sessionManager.getCurrentUser().getUsername();
                if (sessionManager.getCurrentUser().getRoles() != null && sessionManager.getCurrentUser().getRoles().contains("ROLE_MEDECIN")) {
                    reactorName = "Dr.";
                }
                ...
                // Only trigger if clicking on content area, not reactions
                if (e.getTarget() == contentNode || e.getTarget() instanceof javafx.scene.text.TextFlow) {
        ...
        for (String emoji : new String[]{"👍", "❤️", "😂", "😮", "😢"}) {
            ...
            item.setOnAction(e -> {
                String reactorName = sessionManager.getCurrentUser().getUsername();
                if (sessionManager.getCurrentUser().getRoles() != null && sessionManager.getCurrentUser().getRoles().contains("ROLE_MEDCIN")) {
                    reactorName = "Dr.";
                }
    ...
    private void handleEditMessage(Message msg) {
        ...
        if (result.isPresent()) {
            ...
            msg.setOriginalContent(msg.getContent()); // Save original
    ...
    private void navigateToLogin() {
        try {
            ...
            stage.setTitle("Login - WellBalance Messenger");
        ...
        refreshTimeline = new Timeline(
            new KeyFrame(Duration.seconds(3), e -> {
                if (currentConversation != null) {
                    ...
                    if (currentConversation != null) {
                        typingLabel.setVisible(currentConversation.isTypingPatient());
                        typingLabel.setManaged(currentConversation.isTypingPatient());
                    }
                    loadMessages();
                } else {
                    loadDashboardStats();
